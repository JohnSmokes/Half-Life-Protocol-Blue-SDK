...
{
	"hassault/hw_pain3.wav",
	"hassault/hw_pain4.wav",
};
...
void CHAssault::Precache()
{
	...
	PRECACHE_SOUND("hassault/hw_pain3.wav");
	PRECACHE_SOUND("hassault/hw_pain4.wav");
...
void CHAssault::SpinDown()
{
	if (m_ifirestate >= 0)
	{
		...
		STOP_SOUND(ENT(pev), CHAN_WEAPON, "weapons/saw_fire1.wav");
...
void CHAssault::FireGun()
{
	...
	EMIT_SOUND_DYN(ENT(pev), CHAN_WEAPON, "weapons/saw_fire1.wav", 1, ATTN_NORM, 0, 100);
}
...
void CHAssault::StartTask(Task_t* pTask)
{
	...
	case TASK_ASSAULT_SPINUP:
		...
		{
			m_ifirestate = 0;
			STOP_SOUND(ENT(pev), CHAN_WEAPON, "hassault/saw_fire1.wav");
