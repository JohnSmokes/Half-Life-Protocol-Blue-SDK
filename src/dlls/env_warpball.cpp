...
// PS2HL - spawn delay
#define EWB_DELAY_SPAWN 0.5f//1.0f	// PS2 HL has around 1s delay
...
LINK_ENTITY_TO_CLASS(env_warpball_int, CEnvWarpBall)
LINK_ENTITY_TO_CLASS(env_warpball_player, CEnvWarpBall)
...
void CEnvWarpBall::Precache(void)
{
	...
	PRECACHE_MODEL("sprites/portal1.spr");
...
void CEnvWarpBall::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	...
	if (FClassnameIs(pev, "env_warpball"))
	{
	...
	if (FClassnameIs(pev, "env_warpball_int"))
		pev->button = 128;

	CSprite* pSpr;
	if (FClassnameIs(pev, "env_warpball_player"))
	{
		pSpr = CSprite::SpriteCreate("sprites/b-tele1.spr", vecOrigin, TRUE);
		pSpr->pev->frame = 0;
		pSpr->pev->framerate = 20.0;
		pSpr->SetTransparency(kRenderTransAdd, 255, 255, 255, 255, kRenderFxNone);// kRenderFxNoDissipation);
		pSpr->SetScale(1);
	...
	{
		pSpr = CSprite::SpriteCreate("sprites/portal1.spr", vecOrigin, TRUE);
		pSpr->AnimateAndDie(25);
		pSpr->SetTransparency(kRenderTransAdd, 77, 210, 130, 255, kRenderFxNone);// kRenderFxNoDissipation);
		pSpr->SetScale(1.5);
	}
	...
	if (FClassnameIs(pev, "env_warpball_player"))
		pev->nextthink = gpGlobals->time + 1.0f;//pev->frags;
	else
...
void CEnvWarpBall::Think(void)
{
	...
	if (FClassnameIs(pev, "env_warpball_player"))
	{
		...
		edict_t* pentFind = FIND_ENTITY_BY_CLASSNAME(NULL, "env_sparx");
		if (!FNullEnt(pentFind))
		{
			CBaseEntity* pSparx = Instance(pentFind);
			if (pSparx)
				pSparx->pev->frags = 0;
		}
	}
