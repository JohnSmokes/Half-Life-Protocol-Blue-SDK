...
// jay - backported HL2 bob
#define	BOB_MIN	-7.0f
#define	BOB_MAX	4.0f
// stand-ins for g_variables
float	v_verticalBob;
float	v_lateralBob;

// engine function moved here to simplify things
float RemapVal(float val, float A, float B, float C, float D)
{
	return C + (D - C) * (val - A) / (B - A);
}

void V_CalcViewmodelBob(struct ref_params_s* pparams)
{
	...
	float speed = VectorNormalize(pparams->simvel);
	float maxSpd = pparams->movevars->maxspeed;
	// clamp
	if (speed < -maxSpd)
		speed = -maxSpd;
	if (speed > maxSpd)
	...
	float bobOffset = RemapVal(speed, 0.0f, maxSpd, 0.0f, 1.0f);

	bobTime += (pparams->time - lastTime) * bobOffset;
	...
	v_verticalBob = speed * 0.005f;
	v_verticalBob = v_verticalBob * 0.3 + v_verticalBob * 0.7 * sin(cycle);
	// clamp
	if (v_verticalBob < BOB_MIN)
		v_verticalBob = BOB_MIN;
	if (v_verticalBob > BOB_MAX)
		v_verticalBob = BOB_MAX;

	// lateral bob
	cycle = bobTime - (int)(bobTime / cl_bobcycle->value * 2) * cl_bobcycle->value * 2;
	cycle /= cl_bobcycle->value * 2;
	...
	v_lateralBob = speed * 0.005f;
	v_lateralBob = v_lateralBob * 0.3 + v_lateralBob * 0.7 * sin(cycle);
	// clamp
	if (v_lateralBob < BOB_MIN)
		v_lateralBob = BOB_MIN;
	if (v_lateralBob > BOB_MAX)
		v_lateralBob = BOB_MAX;
}

void V_AddViewmodelBob(struct ref_params_s* pparams, Vector& origin, Vector& angles)
{
	Vector	forward, right;
	AngleVectors(angles, forward, right, NULL);

	V_CalcViewmodelBob(pparams);

	VectorMA(origin, v_verticalBob * 0.1f, forward, origin);

	origin[2] += v_verticalBob * 0.1f;

	angles[ROLL] += v_verticalBob * 0.5f;
	angles[PITCH] -= v_verticalBob * 0.4f;
	angles[YAW] -= v_lateralBob * 0.3f;

	VectorMA(origin, v_lateralBob * 0.8f, right, origin);
}
...
void V_CalcNormalRefdef(struct ref_params_s* pparams)
{
	...
	Vector sway;
	sway.x = pparams->forward[0];
	sway.y = pparams->forward[1];
	sway.z = 0;
	...
	for (int i = 0; i < 3; i++)
	{
		...
		view->origin[i] -= 0.5 * fabs(gHUD.lagangle_x) * sway[i];
	}
	//END
	...
	view->origin[2] -= 1;
	V_AddViewmodelBob(pparams, view->origin, view->angles);    // jay - backported HL2 bob 
